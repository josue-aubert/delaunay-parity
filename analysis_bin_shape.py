...
A = {s: np.zeros((n_real, NB)) for s in SETS}      # A[s][real, bin] = sum of shapes in [-1, 1] in that bin
...
for s in SETS:
    np.save(f"{inputdir}/A_shapes_{s}_nb{NB}.npy", A[s])         # shape (n_real, NB)
print(f"saved edges_shapes_nb{NB}.npy, A_shapes_<set>_nb{NB}.npy (n_real x nbins)")
