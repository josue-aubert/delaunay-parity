...
for s in SETS:
    for real in range(n_real):
        ...
        shape = d[:, 0]/(d[:, 1]*d[:, 2]*d[:, 3])   # shape of the chirality
        Ntet = d.shape[0]
        b = [np.digitize(ratio(d, i), edges[i]) - 1 for i in range(3)]   # bin index (0..NB-1) per ratio
        for i in SINGLE:                           # 1D bins
            A1[i][s][real] = np.bincount(b[i], weights=shape, minlength=NB)[:NB] / Ntet
        for (i, j) in PAIRS:                       # 2D bins (flattened)
            flat = b[i] * NB + b[j]
            A2[(i, j)][s][real] = np.bincount(flat, weights=shape, minlength=NB * NB)[:NB * NB] / Ntet
...
for i in SINGLE:
    ...
    plt.title(f"Per-bin paired SNR — shapes, binned by {lab}  ({NB} bins)")
    plt.tight_layout()
    name = f"SNR_shapes_{lab.replace('/', '')}_nb{NB}.png"
...
for (i, j) in PAIRS:
    ...
    plt.title(f"Per-cell paired SNR — shapes, binned by ({li}, {lj})  ({NB}x{NB})")
    plt.tight_layout()
    name = f"SNR_shapes_{li.replace('/', '')}_{lj.replace('/', '')}_nb{NB}.png"
    plt.savefig(name, dpi=150); plt.close()
    print("saved", name)